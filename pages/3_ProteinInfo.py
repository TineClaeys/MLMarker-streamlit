...
has_single_result = "prediction" in st.session_state and "prediction_summed" in st.session_state and "sel_sample" in st.session_state

if not has_batch_results and not has_single_result:
    mark_says("Markverse/cropped_images/Bald Mark reading a book.png", "I don't see any prediction data yet. Run MLMarker on the Home page first!")
...
if "df" not in st.session_state:
    mark_says("Markverse/cropped_images/octopus.png", "No data loaded! Head to the Home page to upload your proteomics data.")
    st.warning("No data found. Please go to the **Home** page and upload your data first.")
    st.stop()
...
if has_batch_results:
    mark_says("Markverse/cropped_images/Mark digging for gold.png", "Multiple samples available! Pick one to explore the protein-level details.")
    sample_options = list(st.session_state.batch_results.keys())
    
    default_idx = 0
    if "protein_selected_sample" in st.session_state and st.session_state.protein_selected_sample in sample_options:
        default_idx = sample_options.index(st.session_state.protein_selected_sample)
    
    selected_sample = st.selectbox(
        "Select sample to explore",
        sample_options,
        index=default_idx,
        key="protein_sample_selector"
    )
    st.session_state.protein_selected_sample = selected_sample
...
st.markdown(f"### Exploring: **{sel_sample}**")
...
selected_tissue = st.selectbox("Select tissue for protein contributions", options=tissues_list,
    help="Choose which tissue's contributing proteins to explore.")
...
shap_filter = st.selectbox("Select model impact", options=["All", "Positive", "Negative"],
    help="Positive: supports tissue. Negative: opposes it.")
...
def display_paginated_proteins_slider(protein_df, selected_tissue, abundance_df, shap_df, page_size=15):
    ...
    subset_proteins = protein_df[protein_df['id'].isin(selected_proteins)].copy()
    ...
    page_df = subset_proteins.iloc[start_idx:end_idx].copy()

    page_df['Protein'] = page_df['id'].apply(lambda x: f'<a href="https://www.uniprot.org/uniprotkb/{x}/entry" target="_blank">{x}</a>')

    if 'Unnamed: 0' in page_df.columns:
        page_df = page_df.drop(columns=['Unnamed: 0'])
    page_df = page_df.drop(columns=['id'])
    page_df = page_df.rename(columns={'entry name': 'Entry', 'protein_names': 'UniProt Description', 'tissue_specificity': 'UniProt Tissue specificity'})
    page_df['UniProt Tissue specificity'] = page_df['UniProt Tissue specificity'].fillna('No tissue specificity information available in UniProt')
...
def display_paginated_proteins(protein_df, selected_tissue, abundance_df, shap_df, page_size=15):
    ...
    if total_pages == 0:
        st.warning("No proteins match the current filter criteria.")
        return
...
with st.sidebar:
    st.markdown(f"**Current Sample:** {sel_sample}")
    st.download_button("Download Tissue level prediction", prediction_summed.to_csv().encode(), f"prediction_summed_{sel_sample}.csv", "text/csv")
    st.download_button("Download Protein level prediction", prediction_df.to_csv().encode(), f"prediction_{sel_sample}.csv", "text/csv")
